...
// ─── Main B-Sides Page ─────────────────────────────────────────────────────────
export default function BSidesPage() {
  ...
  const virtualAlbum = useMemo(
    () => ({
      ...
      // coverPath intentionally empty — per-track artwork comes from /api/bsides/artwork
      // The player's activeCoverPath logic handles B-sides specially.
      coverPath: "",
      accentColor: "hsl(220, 15%, 60%)",
      accentColorRGB: "140, 148, 166",
      description: "A comprehensive collection of Radiohead's B-sides and rarities.",
      producer: "Various",
      label: "Various",
      recordedAt: "Various",
      tracks,
          ...
          >
            ...
            <div>
              <p className="text-sm text-white/50 max-w-lg leading-relaxed">
                {loading ? "..." : `${tracks.length} rare tracks`}, non-album singles, and hidden gems.
              </p>
              <p className="text-xs text-white/30 mt-1">
                {loading ? "Loading tracks..." : `${tracks.length} songs`}
              </p>
            </div>
          </motion.div>
        </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="relative w-10 h-10 rounded bg-white/5 overflow-hidden flex-shrink-0 flex items-center justify-center">
                        <Image
                          src={`/api/bsides/artwork?file=${encodeURIComponent(track.fileName)}`}
                          alt={track.title}
                          fill
